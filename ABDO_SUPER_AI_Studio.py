...
OPENAI_KEY = "ضع_هنا_مفتاحك_الآمن"
...
if not st.session_state["auth"]:
    ...
    st.stop()  # إيقاف الكود حتى تسجيل الدخول
...
uploaded_zip_path = st.file_uploader(
    label="اختر ملف ZIP",
    type=["zip"],
    label_visibility="collapsed"
)
uploaded_file = st.file_uploader(
    label="اختر صورة/فيديو",
...
def autopilot_execute(command, uploaded_file=None, speak=False):
    ...
    try:
        ...
        if "اصنع لعبة" in cmd:
            ...
            output = f"✅ تم تنفيذ الطلب: {command}"
    except Exception as e:
        output = f"[!] خطأ أثناء التنفيذ: {str(e)}"
    
    save_memory(command, output)
    
    # تحويل النص إلى صوت عند الطلب فقط
    if speak:
        try:
